...
public class DTW_DistanceBasic extends EuclideanDistance {
	...
	public double distance(double[] a, double[] b, double cutoff) {
		...
		int m = b.length;
		/*
		 * Parameter 0<=r<=1. 0 == no warp, 1 == full warp generalised for
		 * variable window size
		 */
		matrixD = new double[n][n];
		windowSize = getWindowSize(n);
		/*
		 * //Set all to max. This is necessary for the window but I dont need to
		 * do it all
		 */
		...
		for (int i = 1; i < n; i++) {
			tooBig = true;
			for (int j = 1; j < m; j++) {
				// Find the min of matrixD[i][j-1],matrixD[i-1][j] and
				// matrixD[i-1][j-1]
		...
		// Find the minimum distance at the end points, within the warping
		// window.
	...
	void printPath() {
		...
		int x = n - 1, y = m - 1;
		int count = 0;
		System.out.println(count + "END  Point  = " + x + "," + y + " value ="
				+ matrixD[x][y]);
		while (x > 0 && y > 0) {
			// Look along
			double min = matrixD[x - 1][y - 1];
			if (min <= matrixD[x - 1][y] && min <= matrixD[x][y - 1]) {
				...
				y--;
			count++;
			System.out.println(count + " Point  = " + x + "," + y + " value ="
					+ matrixD[x][y]);
		}
		while (x > 0) {
			x--;
	...
	public static void main(String[] args) {
		...
		// dtw.printPath();
		/*
		 * for(double[] d:dtw.matrixD){ for(double x:d) System.out.print(x+",");
		 * System.out.print("\n"); }
		 */
