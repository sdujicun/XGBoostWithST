...
public abstract class NormalizableDistance implements DistanceFunction,
	...
	public Enumeration listOptions() {
		...
		result.addElement(new Option(
				"\tInvert matching sense of column indices.", "V", 0, "-V"));
	...
	protected double difference(int index, double val1, double val2) {
		...
		case Attribute.NUMERIC:
			if (Utils.isMissingValue(val1) || Utils.isMissingValue(val2)) {
				...
				return (!m_DontNormalize) ? (norm(val1, index) - norm(val2,
						index)) : (val1 - val2);
			}
			...
			if (!instance.isMissing(j)) {
				if (value < ranges[j][R_MIN]) {
					...
					if (value > ranges[j][R_MAX]) { // if this is the first
						...
						ranges[j][R_WIDTH] = ranges[j][R_MAX]
								- ranges[j][R_MIN];
					}
				} else {
	...
	/**
	 * Initializes the ranges of a subset of the instances of this dataset.
	 * Therefore m_Ranges is not set. The caller of this method should ensure
	 * that the supplied start and end indices are valid (start &lt;= end,
	 * end&lt;instList.length etc) and correct.
	 *
	 * @param instList
	 *            list of indexes of the instances
	 * @param startIdx
	 *            start index of the subset of instances in the indices array
	 * @param endIdx
	...
	public double[][] initializeRanges(int[] instList, int startIdx, int endIdx)
		...
		if (m_Data.numInstances() <= 0) {
			...
			// initialize ranges using the first instance
			updateRangesFirst(m_Data.instance(instList[startIdx]), numAtt,
					ranges);
			// update ranges, starting from the second
